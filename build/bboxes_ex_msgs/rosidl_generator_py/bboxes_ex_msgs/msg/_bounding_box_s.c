...
bool bboxes_ex_msgs__msg__bounding_box__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // class_id_int
    PyObject * field = PyObject_GetAttrString(_pymsg, "class_id_int");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->class_id_int = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
...
PyObject * bboxes_ex_msgs__msg__bounding_box__convert_to_py(void * raw_ros_message)
{
  ...
  {  // class_id_int
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->class_id_int);
    {
      int rc = PyObject_SetAttrString(_pymessage, "class_id_int", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
